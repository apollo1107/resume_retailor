...
export default async function handler(req, res) {
  ...
  try {
    ...
    const experienceBulletGuidance = hasPermanent
      ? "Follow **all seven rules** in the prompt header. Summary: (**1**) Add **only a few** JD lines in `details` per role (about **1–4**); if more depth needed, use **same-length `details`** vs `base_bullets` to **expand** each base bullet in place. (**2**) **Never** fewer bullets than profile **`base_bullets`** (append or same-count rewrite). (**3**) **Most** bullets **~two lines**; **shorter** bullets OK. (**4**) In **`experience[0]`** and **`experience[1]`**, include **a few** **≈35+** word depth bullets—**not** every line. (**5**) **No new** `%` / `$` / invented counts unless already in **`base_bullets`**. (**6**) **Dense** bullets + full summary/skills—**no** half-page blank gaps. (**7**) **2–3** pages total—not 1 or 4. **`base_skills` never removed.** Strongest JD fit on **work history #1**."
      : "Follow **all seven rules** in the prompt header. Full `details` per role (**7–10** bullets typical): **most** bullets **~two lines**; **shorter** OK; **`experience[0]`**/**`[1]`**: **a few** **≈35+** word bullets. **No invented metrics** unless in **`base_bullets`** when present. **2–3** pages; fill pages—no sparse blocks. **Tightest JD match** on **work history #1**.";
