...
export default async function handler(req, res) {
  ...
  try {
    ...
    const experienceBulletGuidance = hasPermanent
      ? "Follow **all seven rules** in the prompt header. Summary: (**1**) Add **only a few** JD lines in `details` per role (about **1–4**); if more depth needed, use **same-length `details`** vs `base_bullets` to **expand** each base bullet in place. (**2**) **Never** fewer bullets than profile **`base_bullets`** (append or same-count rewrite). (**3**) Make **every** experience bullet as **long and senior-dense** as facts allow—**professional** stack/domain language, scope, lifecycle, integrations, **qualitative** outcomes. (**4**) **`experience[0]`**/**`[1]`** = **deepest** flagship blocks. (**5**) **No new** `%` / `$` / invented counts unless already in **`base_bullets`**. (**6**) **Dense** layout—**no** half-page gaps. (**7**) **2–3** pages—not 1 or 4. **`base_skills` never removed.** Strongest JD fit on **work history #1**."
      : "Follow **all seven rules** in the prompt header. Full `details` per role (**7–10** bullets typical): **maximize** length on **each** bullet with **professional** technical phrasing; **`experience[0]`**/**`[1]`** especially long and JD-aligned. **No invented metrics** unless in **`base_bullets`**. **2–3** pages; fill pages—no sparse blocks. **Tightest JD match** on **work history #1**.";
